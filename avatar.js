const PokerAvatar = (() => {
  ...
  function renderPicker(config, key, draftCard, error = "", animate = true, scrollState = {}) {
    ...
    `;
    const rankCarousel = root.querySelector(".avatar-rank-carousel");
    const editor = root.querySelector(".avatar-editor");
    if (rankCarousel && Number.isFinite(scrollState.rankScrollLeft)) {
      rankCarousel.scrollLeft = scrollState.rankScrollLeft;
    }
    if (editor && Number.isFinite(scrollState.editorScrollTop)) {
      editor.scrollTop = scrollState.editorScrollTop;
    }
  }
  ...
  function open(options = {}) {
    ...
    return new Promise((resolve) => {
      ...
      function getPickerScrollState() {
        return {
          rankScrollLeft: root.querySelector(".avatar-rank-carousel")?.scrollLeft || 0,
          editorScrollTop: root.querySelector(".avatar-editor")?.scrollTop || 0,
        };
      }
      ...
      function onClick(event) {
        ...
        if (rankButton && pickerDraft) {
          const scrollState = getPickerScrollState();
          pickerDraft.rank = rankButton.dataset.avatarRank;
          renderPicker(config, picker, pickerDraft, "", false, scrollState);
        }
        ...
        if (okButton && picker) {
          ...
          if (isDuplicate(pickerDraft, config[otherKey])) {
            renderPicker(config, picker, pickerDraft, `Cannot select the same card as ${otherKey === "card1" ? "Card One" : "Card Two"}.`, false, getPickerScrollState());
            return;
          }
